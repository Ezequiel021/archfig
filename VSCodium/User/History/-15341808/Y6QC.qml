...
Item {
    ...
    // 2. Extraemos dinámicamente las redes (si existen)
    ...
    // 3. LA LÓGICA DE PRIORIDAD E INTENSIDAD
    property string panelIcon: {
        // Prioridad 1: Ethernet siempre gana si está conectado
        if (ethNet) {
            return Quickshell.iconPath("network-wired-symbolic")
        }

        // Prioridad 2: Evaluamos la señal de Wi-Fi
        if (wifiNet) {
    ...
    ColumnLayout {
        ...
        // ... (El resto de tu código de las tarjetas Rectangle para ethNet y wifiNet se mantiene idéntico) ...
