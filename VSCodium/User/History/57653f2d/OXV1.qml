...
Item {
    ...
    function updateFilter() {
        // Convert the QML list into a standard JS array first 
        // to prevent [index] and .length lookup failures later
        let allApps = [];
        for (let i = 0; i < DesktopEntries.applications.length; i++) {
            allApps.push(DesktopEntries.applications[i]);
        }
