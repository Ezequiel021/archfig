...
Item {
    ...
    function updateFilter() {
        // Directly grab the raw JS array from the Quickshell model's "values" property
        let allApps = DesktopEntries.applications.values;
        
        // Safety check: if the model hasn't loaded its values yet, bail out safely
        if (!allApps) {
            filteredApps = [];
            return;
        }
        ...
        if (term === "") {
            // Because allApps is now a true JS array, we can safely assign it directly
