...
\[
\boxed{\Omega + \Delta \text{ es abierto.}}
\]

\textbf{Demostración alternativa (más elegante):} 

Otra forma de verlo es observando que la suma de conjuntos se puede expresar como una unión de traslaciones:
\[
\Omega + \Delta = \bigcup_{\y \in \Delta} (\Omega + \y),
\]
donde $\Omega + \y := \{\x + \y : \x \in \Omega\}$ es el conjunto $\Omega$ trasladado por el vector $\y$. Cada traslación $\Omega + \y$ es abierta, ya que la traslación es un homeomorfismo de $\R^n$ en sí mismo y preserva los conjuntos abiertos. Como la unión arbitraria de conjuntos abiertos es abierta, se concluye que $\Omega + \Delta$ es abierto.

De manera equivalente, también se puede escribir
\[
\Omega + \Delta = \bigcup_{\x \in \Omega} (\x + \Delta),
\]
donde $\x + \Delta := \{\x + \y : \y \in \Delta\}$, y cada $\x + \Delta$ es abierto por ser una traslación de $\Delta$.
