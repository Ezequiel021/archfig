...
\textbf{Solución: } Veamos que \(\norm{\x}_{\max}\) es una norma:
    \begin{itemize}
        \item Por definición del valor absoluto, \(\abs{x_i} \geq 0\) para toda \(i \in [n]\), y ya que \(\max\{\abs{x_1}, \dots, \abs{n}\} \geq \abs{x_i}\), se sigue que \(\max\{\abs{x_1}, \dots, \abs{n}\} \geq 0\). Por lo tanto, \(\norm{\x}_{\max} \geq 0\) para toda \(\x\).
...
\begin{itemize}
    ...
    \]
    por la desigualdad del triángulo en \(\R\), tenemos las desigualdades 
    \[
    \sum_{i = 1}^{n}\abs{x_i} \geq \abs{\sum_{i = 1}^{n}x_i} \quad \text{y} \quad \sum_{i = 1}^{n}\abs{y_i} \geq \abs{\sum_{i = 1}^{n}y_i}.
    \]
    Sustituyendo, 
    \[
    \sum_{i = 1}^{n}\abs{x_i} + \sum_{i = 1}^{n}\abs{y_i} \geq \abs{\sum_{i = 1}^{n}x_i} + \abs{\sum_{i = 1}^{n}y_i} \geq \abs{\sum_{i = 1}^n x_i + \sum_{i = 1}^n y_i}
    \]
