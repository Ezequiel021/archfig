...
\textbf{Solución: } Recordemos las siguientes definiciones:
\begin{itemize}
    \item Un conjunto $\Omega \subseteq \R^n$ es cerrado si $\Omega = \overline{\Omega}$, donde $\overline{\Omega}$ denota la cerradura de $\Omega$.
    \item La frontera de $\Omega$ se define como $\partial \Omega = \overline{\Omega} \setminus \Int(\Omega)$.
    \item El interior de un conjunto $A$, denotado por $\Int(A)$, es el conjunto de puntos $\x \in A$ para los cuales existe $\varepsilon > 0$ tal que $B(\x, \varepsilon) \subseteq A$.
\end{itemize}

Supongamos que $\Omega$ es cerrado. Entonces $\overline{\Omega} = \Omega$, y la frontera de $\Omega$ es
\[
\partial \Omega = \Omega \setminus \Int(\Omega).
\]

Demostraremos que $\Int(\partial \Omega) = \emptyset$ por contradicción.

\textbf{Demostración por contradicción:}

Supongamos que $\Int(\partial \Omega) \neq \emptyset$. Entonces existe un punto $\x \in \Int(\partial \Omega)$ y un $\varepsilon > 0$ tal que
\[
B(\x, \varepsilon) \subseteq \partial \Omega.
\]

Como $\partial \Omega = \Omega \setminus \Int(\Omega) \subseteq \Omega$, se tiene que
\[
B(\x, \varepsilon) \subseteq \Omega.
\]
Pero esto significa que $\x$ es un punto interior de $\Omega$, es decir, $\x \in \Int(\Omega)$.

Por otro lado, como $B(\x, \varepsilon) \subseteq \partial \Omega = \Omega \setminus \Int(\Omega)$, se tiene que
\[
B(\x, \varepsilon) \cap \Int(\Omega) = \emptyset.
\]
En particular, $\x \notin \Int(\Omega)$, ya que si $\x \in \Int(\Omega)$, entonces $\x \in B(\x, \varepsilon) \cap \Int(\Omega)$, lo cual contradice que dicha intersección sea vacía.

Hemos llegado a una contradicción: por un lado, $\x \in \Int(\Omega)$, y por otro lado, $\x \notin \Int(\Omega)$. Por lo tanto, nuestra suposición inicial de que $\Int(\partial \Omega) \neq \emptyset$ debe ser falsa.

Concluimos que
\[
\boxed{\Int(\partial \Omega) = \emptyset.}
\]

\textbf{Demostración alternativa (más directa):}

Sea $\x \in \partial \Omega$ un punto arbitrario de la frontera. Demostraremos que $\x$ no puede ser un punto interior de $\partial \Omega$.

Por definición de frontera, toda bola abierta centrada en $\x$ contiene tanto puntos de $\Omega$ como puntos de $\R^n \setminus \Omega$.

Supongamos, para buscar una contradicción, que existe $\varepsilon > 0$ tal que $B(\x, \varepsilon) \subseteq \partial \Omega$. Entonces, en particular, $B(\x, \varepsilon) \subseteq \overline{\Omega} = \Omega$ (ya que $\Omega$ es cerrado). Esto significa que $\x \in \Int(\Omega)$.

Pero $\Int(\Omega) \cap \partial \Omega = \emptyset$, ya que $\partial \Omega = \overline{\Omega} \setminus \Int(\Omega)$. Esto contradice que $B(\x, \varepsilon) \subseteq \partial \Omega$ y que $\x \in B(\x, \varepsilon)$.

Por lo tanto, ningún punto de $\partial \Omega$ puede ser un punto interior de $\partial \Omega$, lo que implica que $\Int(\partial \Omega) = \emptyset$.
...
\item Supongamos que $a_1,\ldots,a_n,b_1,\ldots,b_n\in \R$ son tales que $a_j<b_j$ para toda $j=1,\ldots,n$. Hemos definido el rectángulo cerrado (abierto) por
\[ R = [a_1,b_1]\times \cdots\times [a_n,b_n], \]
o bien,
\[ R = (a_1,b_1)\times \cdots\times (a_n,b_n). \]
En cualquier caso, la \textsf{diagonal} de $R$ se define por:
...
R = [a_1,b_1]\times \cdots\times [a_n,b_n]
\]
un rectángulo cerrado en $\R^n$, donde $a_j < b_j$ para toda $j = 1, \ldots, n$. Debemos demostrar tres propiedades: que $R$ es cerrado, acotado y convexo.

\textbf{1. $R$ es cerrado.}

\textit{Demostración 1 (usando funciones continuas):}

Para cada $j \in \{1, \ldots, n\}$, definimos las funciones proyección
\[
\pi_j : \R^n \to \R, \qquad \pi_j(x_1, \ldots, x_n) = x_j.
\]
Cada $\pi_j$ es una función continua. Observamos que
\[
R = \bigcap_{j=1}^{n} \pi_j^{-1}([a_j, b_j]).
\]
En efecto, un punto $\x = (x_1, \ldots, x_n) \in \R^n$ pertenece a $R$ si y solo si $a_j \leq x_j \leq b_j$ para toda $j$, lo que equivale a que $\pi_j(\x) \in [a_j, b_j]$ para toda $j$.

Como cada intervalo $[a_j, b_j]$ es cerrado en $\R$ y cada $\pi_j$ es continua, cada imagen inversa $\pi_j^{-1}([a_j, b_j])$ es cerrada en $\R^n$. La intersección finita de conjuntos cerrados es cerrada, por lo tanto, $R$ es cerrado.

\textit{Demostración 2 (usando sucesiones):}

Sea $(\x_k)_{k \in \N}$ una sucesión de puntos en $R$ que converge a un punto $\x \in \R^n$. Escribimos $\x_k = (x_{k,1}, \ldots, x_{k,n})$ y $\x = (x_1, \ldots, x_n)$. Como $\x_k \in R$, se tiene que
\[
a_j \leq x_{k,j} \leq b_j \quad \text{para toda } j = 1, \ldots, n \text{ y toda } k \in \N.
\]
La convergencia $\x_k \to \x$ en $\R^n$ implica la convergencia componente a componente, es decir, $x_{k,j} \to x_j$ para cada $j$. Tomando límites en las desigualdades anteriores y usando que los límites preservan desigualdades no estrictas, obtenemos
\[
a_j \leq x_j \leq b_j \quad \text{para toda } j = 1, \ldots, n.
\]
Por lo tanto, $\x \in R$. Esto demuestra que $R$ contiene a todos sus puntos límite, y por consiguiente, $R$ es cerrado.

\textbf{2. $R$ es acotado.}

Recordemos que un conjunto $A \subseteq \R^n$ es acotado si existe $M > 0$ tal que $\norm{\x} \leq M$ para toda $\x \in A$.

Sea $\x = (x_1, \ldots, x_n) \in R$ un punto arbitrario. Entonces $a_j \leq x_j \leq b_j$ para toda $j$. Definimos
\[
c_j = \max\{\abs{a_j}, \abs{b_j}\} \geq 0.
\]
Entonces $\abs{x_j} \leq c_j$ para toda $j$. Usando la norma euclidiana, tenemos:
\[
\norm{\x}^2 = \sum_{j=1}^{n} x_j^2 = \sum_{j=1}^{n} \abs{x_j}^2 \leq \sum_{j=1}^{n} c_j^2.
\]
Sea $M = \sqrt{\sum_{j=1}^{n} c_j^2} > 0$. Entonces
\[
\norm{\x} \leq M
\]
para toda $\x \in R$. Por lo tanto, $R$ es acotado.

Alternativamente, podemos usar la diagonal de $R$. Sea $\mathbf{a} = (a_1, \ldots, a_n)$ y $\mathbf{b} = (b_1, \ldots, b_n)$. Para cualquier $\x \in R$, se tiene que $\abs{x_j - a_j} \leq b_j - a_j$ para toda $j$. Entonces,
\[
\norm{\x - \mathbf{a}}^2 = \sum_{j=1}^{n} (x_j - a_j)^2 \leq \sum_{j=1}^{n} (b_j - a_j)^2 = \diag(R)^2,
\]
por lo que $\norm{\x - \mathbf{a}} \leq \diag(R)$. Usando la desigualdad triangular,
\[
\norm{\x} = \norm{(\x - \mathbf{a}) + \mathbf{a}} \leq \norm{\x - \mathbf{a}} + \norm{\mathbf{a}} \leq \diag(R) + \norm{\mathbf{a}},
\]
lo que también demuestra que $R$ es acotado.

\textbf{3. $R$ es convexo.}

Recordemos que un conjunto $A \subseteq \R^n$ es convexo si para cualesquiera dos puntos $\x, \y \in A$ y cualquier $t \in [0,1]$, se cumple que
\[
(1-t)\x + t\y \in A.
\]

Sean $\x = (x_1, \ldots, x_n)$ y $\y = (y_1, \ldots, y_n)$ dos puntos arbitrarios de $R$. Entonces, para toda $j = 1, \ldots, n$, se tiene:
\[
a_j \leq x_j \leq b_j \quad \text{y} \quad a_j \leq y_j \leq b_j.
\]
Sea $t \in [0,1]$ arbitrario. Definimos
\[
\mathbf{z} = (1-t)\x + t\y = ((1-t)x_1 + ty_1, \ldots, (1-t)x_n + ty_n).
\]
Para cada componente $j$, como $0 \leq t \leq 1$ y $1-t \geq 0$, tenemos:
\[
z_j = (1-t)x_j + ty_j \geq (1-t)a_j + ta_j = ((1-t) + t)a_j = a_j,
\]
y
\[
z_j = (1-t)x_j + ty_j \leq (1-t)b_j + tb_j = ((1-t) + t)b_j = b_j.
\]
Por lo tanto, $a_j \leq z_j \leq b_j$ para toda $j$, lo que implica que $\mathbf{z} \in R$.

Como $\x, \y \in R$ y $t \in [0,1]$ eran arbitrarios, concluimos que $R$ es convexo.

\textbf{Conclusión:}

Hemos demostrado que todo rectángulo cerrado
\[
R = [a_1,b_1]\times \cdots\times [a_n,b_n]
\]
es un subconjunto cerrado, acotado y convexo de $\R^n$.
