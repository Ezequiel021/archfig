...
Item {
    ...
    Timer {
        id: hideTimer
        interval: 350 // Retraso en milisegundos (ajusta a tu gusto)
        repeat: false
        onTriggered: {
            // Nos aseguramos de cerrar solo si este sigue siendo el menú activo
        ...
        Repeater {
            ...
            delegate: Item {
                ...
                MouseArea {
                    ...
                    onEntered: {
                        // Detenemos el timer por si el usuario saca y regresa el cursor rápidamente
                        hideTimer.stop()

                        // Verificamos si necesitamos abrir el menú
                        if (systemTrayRoot.activeMenu !== modelData.menu || !systemTrayRoot.isMenuShown) {
                    ...
                    // 3. Lógica al sacar el cursor
                    onExited: {
                        // Iniciamos la cuenta regresiva antes de cerrar
                        hideTimer.start()
                    }

                    // 4. Temporizador anidado para el retraso
                }
            }
        }
    }
